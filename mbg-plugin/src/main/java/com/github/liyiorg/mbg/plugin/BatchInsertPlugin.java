...
public class BatchInsertPlugin extends PluginAdapter {
    ...
    public boolean sqlMapDocumentGenerated(Document document, IntrospectedTable introspectedTable) {
        ...
        if (!readonly) {
            ...
            if (introspectedTable.getGeneratedKey() != null) {
                ...
                for(IntrospectedColumn n : introspectedTable.getAllColumns()){
                	if(!n.getActualColumnName().equals(generatedKeyColumn)){
                		columns.add(n);
                	}
                }
    ...
    private XmlElement batchInsertXmlElement(String tableName, List<IntrospectedColumn> columns) {
        ...
        for (int i = 0; i < columns.size(); i += 4) {
            ...
            if (i + 4 < columns.size()) {
                stringBuilderInto.append(System.lineSeparator());
            }
            stringBuilderInto.append("      ");
        }
        ...
        for (int i = 0; i < columns.size(); i += 3) {
            ...
            if (i + 3 < columns.size()) {
                stringBuilderValues.append(System.lineSeparator());
            }
            stringBuilderValues.append("      ");
        }
